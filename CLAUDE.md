...
10. **No `backdrop-filter`/blur on mobile.** A global rule in `src/index.css` (`@media (max-width:767px){ *:not(.allow-blur),::before,::after{ backdrop-filter:none !important } }`) disables it below 768px. ONE sanctioned exemption: `.allow-blur` on the quote-sheet scrim (`.scrim-blur`) — safe because its blur radius animates to 0 before the surface unmounts, sidestepping the unmount blank-flash that motivated the ban. Any new `.allow-blur` use must follow the same animate-to-zero-before-unmount contract and be verified on a real iPhone. On iOS Safari, blurred surfaces force a re-raster of everything behind them, and mounting/toggling them (nav drawer, quote sheet, sticky CTA, scroll-triggered navbar) blanks and repaints the whole page. Desktop keeps frosted glass, capped at **10px** (`backdrop-blur-[10px]`, not the old `-xl`/24px). Decorative `filter: blur()` glow orbs (`[class*="blur-["].rounded-full`) are also `display:none` on mobile.
...
13. **A commercial page must ship a real `<form>` in its PRERENDERED HTML.** The QuoteSheet popup is mount-on-open, so a page whose only conversion path is `openQuoteSheet()` crawls as zero `<form>`/`<input>`/`<select>` — it reads to a search engine as an *article about* a service, not a *landing page for* one. `/services`, `/services/green-pool-recovery` and `/services/storm-cleanup` were all in that state; they now render `src/components/ServiceQuoteForm.tsx` statically. Verify after a build with `grep -c '<form' dist/<route>/index.html`. Two follow-on rules: (a) that component takes NO Framer Motion, because an entrance animation bakes `style="opacity:0;transform:…"` into the static HTML and re-serializes differently on the client (React #418 — the homepage `QuoteForm` needs `initial={false}` for exactly this); (b) give each page its own copy AND its own qualifying fields — near-duplicate boilerplate repeated across a site is discounted, so the green page asks how long it has been green and the storm page asks what is in the water. `functions/api/contact.ts` renders unknown payload keys as extra email rows, so a new field needs no backend change — but a new `service` **slug** does need adding to `humanizeService()` there.
...
- **Keep the nav menu + quote sheet animating on mobile.** The menu is **portaled to `document.body`** (`createPortal` in `Navbar.tsx`) so it renders *outside* `.force-static-motion`; the quote sheet renders from `QuoteSheetProvider` at the app root, already outside it. Both animate via CSS classes (`.overlay-panel-fade` / `.overlay-panel-bottom` / `.scrim-blur`), which the force-visible rule's inline-style selector never matches — no Framer, no nested `MotionConfig` in either overlay.
...
- **Motion = composited CSS transitions, not Framer Motion.** `.overlay-scrim` / `.scrim-blur` / `.overlay-panel-bottom` (sheet slide) / `.overlay-panel-fade` (menu fade + zoom) in `src/index.css` animate `transform`/`opacity`/`backdrop-filter` via CSS transition. A JS tween (`m.*` animating `x`/`y`, or a spring) runs on the main thread and stuttered; these transitions run on the **compositor**, so they stay smooth even while the main thread is busy. The old `m.*`/`AnimatePresence`/`MotionConfig` were removed from both overlays.
- **Nav menu is PRE-MOUNTED** (`src/components/Navbar.tsx`): mounted once after hydration (gated on a `hydrated` state so it's never in the prerendered HTML) and parked at opacity:0 with `visibility:hidden` (`.nav-drawer`, flipped after the close transition so the parked panel costs nothing at paint time). Opening just toggles `is-open` → no on-tap React mount, no animation-frame wait → instant. When closed it's also `inert` + `pointer-events:none`.
- **Quote sheet is MOUNT-ON-OPEN** (`src/components/QuoteSheet.tsx`) via `useOverlayTransition` (mount → next frame add `is-open` → transition; remove → unmount after the duration). It is NOT pre-mounted at load: it hosts the heavy `QuoteChooser` form, so keeping it always-mounted would tax every page load. It IS warmed on intent: triggers call `warm()` from `useQuoteSheet()` on `pointerdown` (currently the mobile-menu CTA), which mounts the sheet hidden (`pointer-events-none` + `inert`) so the open transition never competes with the form's mount — mounting on tap dropped the entrance's first frames and the slide read as "it just appears". Warmed is sticky: after first use the sheet stays mounted across open/close.
