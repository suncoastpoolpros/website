// FAQ source-of-truth for the St. Pete Beach page. Kept in its own tiny module
// so the page chunk can include it for JSON-LD without pulling in the rest of
// the below-the-fold React tree. Content is specific to St. Pete Beach —
// barrier-island salt air, heavy sun/use, and the mix of full-time residents,
// second homes, and rentals along Gulf Blvd / Pass-a-Grille.
export const stPeteBeachFaqs = [
  {
    q: 'Do you service pools right on the Gulf in St. Pete Beach?',
    a: "Yes — Gulf-front and bayside pools are most of what we do out here. A beach pool just takes more abuse than an inland one: salt air, blowing sand, and that full afternoon sun all at once. So we run a slightly heavier chemistry and equipment-check routine on them. Gulf Blvd, Pass-a-Grille, one of the bayside streets off Boca Ciega — wherever you are, the weekly visit is built around how a barrier-island pool actually wears.",
  },
  {
    q: 'How does salt air affect my pool equipment near the beach?',
    a: "More than people expect. Salt air corrodes heater cabinets, pump hardware, light rings, and screws far faster than it would a few miles inland, and a salt-chlorine cell scales up on top of that. So every visit, we look at the pad — check for corrosion, rinse off the salt, flag anything starting to go. Caught early it's usually a cheap part. Ignored, it's a failed pump on a holiday weekend and a much bigger bill.",
  },
  {
    q: 'Can you keep my pool ready for guests and renters?',
  ...
  {
    q: 'How much does pool service cost in St. Pete Beach?',
