...
const CHECKS = [
  ...
  ['src/components/admin/tierPresets.ts', /const ALL_PRESET_ROWS = \(\) =>/,
   'sync must derive its generated-row set from the builders, not from anchors'],
  ['src/components/admin/tierPresets.ts', /\[\.\.\.rebuilt, \.\.\.head\.filter\(\(b\) => !isGenerated\(b\)\)\]/,
   'an edited card must re-emit preset rows and keep only typed lines'],
